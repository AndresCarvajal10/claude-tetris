...
- `game.js` — all game logic, structured around a small set of core concepts:
  ...
  - **Local records**: `localStorage` key `tetris-records` = `{ top: [{name, score, lines, level, date}] (max `MAX_RECORDS`=5, desc), bestCombo, maxLines }`; player name remembered under `tetris-player-name`. Combo = consecutive non-bomb locks clearing >=1 line (tracked in `lockPiece`). A `#start-screen` overlay shows the table at load; the game only starts via `init()` (Jugar button / Enter). `endGame` calls `finishRecords` (name form if the score qualifies, else table), `submitName` inserts and highlights the row, `resetRecords` clears with `confirm()`. Corrupt JSON falls back to defaults. Keydown is ignored while typing in the name input.
...
`COLS`, `ROWS`, `BLOCK` (cell pixel size), `COLORS`, `LINE_SCORES`, `dropInterval`, `BOMB_CHANCE` (bomb piece spawn probability), `BOMB_CELL_SCORE`, `MAX_RECORDS` / `NAME_MAX` (records table size, name length). If `COLS`/`ROWS`/`BLOCK` change, update the `width`/`height` attributes of `<canvas id="board">` in `index.html` to match (`COLS × BLOCK` by `ROWS × BLOCK`).
